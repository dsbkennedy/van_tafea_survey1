...
yes_no_factor_names <- c('head_of_household',
                         'goods_electricity',
                         'goods_radio',
                         'goods_television',
                         'goods_cellphone',
...
checked_unchecked_factor <- c('walls_strawbamboo',
                              'walls_brick',
                              'walls_wood',
                              'walls_ironsheet',
                              'walls_other',
...
bednet_vars <- c('bed_net_raining_season_all', 'bed_net_raining_season_one')

drinking_water_vars <- c('drinking_water_source', 'washing_water_source')

education_levels <- c('Neva go lo skul',
...
education_labels <- c("Never went to school",
...
occupation_levels <- c("Farma",
                       ...
                       "Narafala wok",
                       "No save")
occupation_labels <- c("Farmer","Fisherman/woman" ,"Clerk/administration" ,"Health worker" , 
                       "Teacher", "Police","Self-employed/business owner (including selling at the market)" ,
                       "Labourer/manual worker (e.g. builder)" , "Domestic worker (e.g house keeper, home maker)" ,
                       "Student" , "Unemployed" , "Other" , "Don't know")

income_levels <- c("Less than 50,000 VTU" , 
                   "50,000 - 99,999 VTU" ,
                   "100,000 - 199,999 VTU" ,
                   "Bitim 200,000 VTU" , "No save")

income_labels <- c("Less than 50,000 VTU" , 
                   "50,000 - 99,999 VTU" ,
                   "100,000 - 199,999 VTU" ,
                   "200,000 VTU or more" , "Dont know")

drinking_water_levels <- c('Piped wota lo hao o yard', 'Public tap o standpaep o Tubewell/borehole', 
                           'Protected dug well', 
...
drinking_water_labels <- c("Piped water to your house or yard" ,"Public tap or standpipe or Tubewell/borehole" , "Protected dug well",
                           "Protected spring" , "Rainwater (collected into tank)" ,"Unprotected dug well" ,"Unprotected spring" ,
                           "Surface water (e.g, lake, river, stream, pond, canals, irrigation ditches)" , "Other, specify:" ,"Don't know")

hyg_levels <- c("Sink/tap klosap lo toilet", "Sink/tap klosap lo kitchen",
               "Sink/tap stap lo narafala lokesen", "Mobile (backet/jag/kettle)",
               "Narafala, spesifae", "No save chekem / no allow blo chekem")

hyb_labels <- c( "Sink/tap close to toilet" ,"Sink/tap close to the kitchen" ,"Sink/tap other location",
                 "Mobile (bucket/jug/kettle)" , "Other (Specify)" ,"None/no permission to see")

toilet_levels <- c('Flash toilet',
                   'Pit latrine',
                   'Compos toilet',
                   'Backet latrine',
                   'Hanging latrine',
...
toilet_labels <- c("Flush toilet","Pit latrine", "Composting toilet" ,
                   "Bucket latrine","Hanging latrine" ,"Other (Specify)" ,"No permission to see" , "No toilet")

pit_levels <- c('No flash (direct pit)', 
                'Flash wetem wota (offset pit)',
                'Flash wetem wota (flash lo narapls)')

pit_labels <- c('Does not flush (direct pit)',
                'Flush with water (offset pit)',
                'Flush with water (flush elsewhere)')

bednet_levels <- c('Neva', 'Sam naet', 'Fulap naet', 'Everi naet')
bednet_labels <- c("Never" , "Some nights" , "Most nights" , "Every night")

bednet_lastnight_levels <- c('No - nokat wan', 'Yes - samfala', 'Yes  - everiwan', 'Nosave')
bednet_lastnight_labels <- c("No - none of us" ,"Yes - some of us" ,"Yes - all of us" ,"Don't know'")

skin_varlist <- c('scabies_scratching_24',
                  ...
                  'yaws_village_arrive_6m',
                  'yaws_village_travel_6m',
                  'yaws_dpp_line1',
                  'yaws_dpp_line2',
                  'yaws_dpp_linec',
                  'yaws_swab_collected',
...
ulcer_location_levels <- c("Left upper arm",
                           "Left lower arm",
                           "Left upper leg",
                           "Left lower leg",
                           "Left ankle" ,
...
ulcer_location_labels <- c("Left upper arm",
                           "Left lower arm",
                           "Left upper leg",
                           "Left lower leg",
                           "Left ankle",
...
dpp_result_levels <- c('Positif: Aktive Yaws (3 line ikam aot: 1, 2 & C)',

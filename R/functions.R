risk_factor_processing_fn <- function() {
  ...
  (
    risk_factor_processed <- risk_factor_raw %>%
      ...
      mutate(area_council_fct=factor(case_when(village_fct=='karimasanga' ~ 'south_west_tanna', 
...
form2_processing_fn <- function() {
  ...
  mutate(f2_azith_any_fct = case_when(f2_azith_1tab_fct == 'yes' ~ 'yes',
    ...
    mutate(f2_pm_any_fct = factor(case_when(f2_pm_2m_fct == 'yes' ~ 'yes',
                                            f2_pm_2m_12yr_fct == 'yes' ~ 'yes',
                                            f2_pm_other_fct == 'yes' ~ 'yes'), levels=c('yes', 'no')))
  ...
  form2_1_processed_upd <- form2_combine_fill %>% filter(data_entry==1) %>% ungroup()
  form2_2_processed_upd <- form2_combine_fill %>% filter(data_entry==2) %>% ungroup()
...
form2_data_cleaning <- function(data1, data2) {
  
  form2_consent_update <- data1 %>% filter((is.na(f2_consent_fct) | f2_consent_fct=='no') & f2_flag==1) %>% 
    mutate(f2_consent_fct_update=factor(case_when(f10_flag==1 | f11_results_flag==1 | skin_exam_flag==1 ~ 'yes', 
                                        ...
                                        levels=c('yes', 'no'))) %>% 
    dplyr::select(mda_code, consent_update = f2_consent_fct_update) %>% 
    filter(!is.na(consent_update))

form2_present_update <- data1 %>% filter(is.na(f2_present_fct) & f2_flag==1) %>% 
  mutate(f2_present_fct_update=factor(case_when(f10_flag==1 | f11_results_flag==1 | skin_exam_flag==1 ~ 'yes', 
                                      ...
                                      levels=c('yes', 'no'))) %>% 
  dplyr::select(mda_code, present_update = f2_present_fct_update) %>% 
  filter(!is.na(present_update))

f2_questionnaire_update <- data1 %>% 
...
form2_working_data_update <- data2 %>% 
  ...
  left_join(form2_present_update, by='mda_code') %>% 
  mutate(f2_present_updated=factor(case_when(present_update=='yes' ~ 'yes', 
                                             TRUE ~ as.character(f2_present_fct)),
                                   levels=c('yes', 'no')))  %>% 
  ...
  left_join(form2_consent_update, by='mda_code') %>% 
  mutate(f2_consent_updated=factor(case_when(consent_update=='yes' ~ 'yes', 
                                             TRUE ~ as.character(f2_consent_fct)), 
                                   levels=c('yes', 'no')))

return(form2_working_data_update)
}
...
stool_qpcr_results_fn <- function() {
...
stool_results_processing_fn <- function(data1, data2) {

  stool_results_processsed <- data1 %>%
  dplyr::filter(sample_yn == 1 | qPCR_results_flag == 1) %>%
  full_join(data2, by = 'mda_code') %>%
  dplyr::select(
    qPCR_unimelb_id,
    mda_code,
    f11_area_council_fct,
  ...
  mutate(
    f11_result_ascaris = case_when(
      f11_ascaris_epg > 0 ~ 'pos',
      f11_ascaris_epg == 0 ~ 'neg',
      TRUE ~ 'no result'
    )
  ) %>%
  mutate(
    f11_result_trichuris = case_when(
      f11_trichuris_epg > 0 ~ 'pos',
      f11_trichuris_epg == 0 ~ 'neg',
      TRUE ~ 'no result'
    )
  ) %>%
  mutate(
    f11_result_hookworm = case_when(
      f11_hookworm_epg > 0 ~ 'pos',
      f11_hookworm_epg == 0 ~ 'neg',
      TRUE ~ 'no result'
    )
  ) %>%
  mutate(
    qPCR_result_hookworm = case_when(
      rowSums(across(
        c(
  ...
  return(stool_results_processsed)
}

form3.1_10_merge_fn <- function(data1, data2) {
  
  data1 %>%
    full_join(data2, by = 'mda_code') %>%
    filter(is.na(f10_flag) | is.na(f31_flag)) %>%
    mutate(village = coalesce(f31_village_fct, f10_village_fct)) %>%
    mutate(area_council = coalesce(
      f31_area_council_fct, f10_area_council_fct
    )) %>%
...
skin_exam_summary_table_fn <- function(data) {
  
  skin_exam_results_age_group_tbl <- data %>%
    dplyr::select(
      skin_exam_age_group,
    ...
    mutate(prop = map2(n, total, ~ prop.test(.x, .y, conf.level=0.95) %>%
                         broom::tidy())) %>%
    unnest(prop) %>% 
    dplyr::select(1:6, conf.low, conf.high) %>% clean_names() %>% 
